...
class Activity {
    ...
    isLimitExceeded(domain, tab) {
        if (setting_restriction_list !== undefined && setting_restriction_list.length > 0) {
            let item = setting_restriction_list.find(o => o.url.isMatch(domain));
            if (item !== undefined) {
                let data = tab.days.find(x => x.date == todayLocalDate());
                if (data !== undefined) {
                    let todayTimeUse = data.summary;
    ...
    isNeedNotifyView(domain, tab){
        if (setting_notification_list !== undefined && setting_notification_list.length > 0) {
            let item = setting_notification_list.find(o => o.url.isMatch(domain));
            if (item !== undefined) {
                let today = todayLocalDate();
                let data = tab.days.find(x => x.date == today);
                if (data !== undefined) {
                    let todayTimeUse = data.summary;
