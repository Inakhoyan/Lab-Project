...
let currentTab;
let isNeedDeleteTimeIntervalFromTabs = false;
let activity = new Activity();
let storage = new LocalStorage();
let deferredRestrictionsList;

let setting_black_list;
let setting_restriction_list;
let setting_interval_save;
let setting_interval_inactivity;
let setting_view_in_badge;
let setting_block_deferral;
let setting_dark_mode;
let setting_notification_list;
let setting_notification_message;

let isHasPermissioForYouTube;
let isHasPermissioForNetflix;
let isHasPermissioForNotification;
...
function setBlockPageToCurrent(currentUrl) {
    let blockUrl = chrome.runtime.getURL("block.html") + '?url=' + currentUrl;
...
function executeScriptYoutube(callback, activeUrl, tab, activeTab) {
    chrome.tabs.executeScript({ code: "let videoElement = document.getElementsByTagName('video')[0]; (videoElement !== undefined && videoElement.currentTime > 0 && !videoElement.paused && !videoElement.ended && videoElement.readyState > 2);" }, (results) => {
