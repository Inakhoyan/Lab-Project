...
function toCsv(tabsData) {
    let str = 'domain,date,time(sec)\r\n';
    for (let i = 0; i < tabsData.length; i++) {
        for (let y = 0; y < tabsData[i].days.length; y++) {
            let line = tabsData[i].url + ',' + new Date(tabsData[i].days[y].date).toLocaleDateString() + ',' + tabsData[i].days[y].summary;
            str += line + '\r\n';
        }
    }
...
function actionAddRectrictionToList(newSite, newTime) {
    if (!isContainsRestrictionSite(newSite)) {
        let restriction = new Restriction(newSite, newTime);
...
function actionAddNotifyToList(newSite, newTime) {
    if (!isContainsNotificationSite(newSite)) {
        let notify = new Notification(newSite, newTime);
...
function addDomainToEditableListBox(entity, elementId, actionEdit, actionDelete, actionUpdateTimeFromList, actionUpdateList) {
    ...
    let li = document.getElementById(elementId).appendChild(li);
...
function deleteRestrictionSite(e) {
    let targetElement = e.path[1];
    let itemValue = targetElement.querySelector("[name='domain']").value;
    let item = restrictionList.find(x => x.url.isMatch(itemValue));
...
function deleteNotificationSite(e) {
    let targetElement = e.path[1];
    let itemValue = targetElement.querySelector("[name='domain']").value;
    let item = notifyList.find(x => x.url.isMatch(itemValue));
...
function actionEditSite(e, actionUpdateTimeFromList, actionUpdateList) {
    let targetElement = e.path[1];
    let domainElement = targetElement.querySelector('[name="domain"]');
    let timeElement = targetElement.querySelector('[name="time"]');
    if (timeElement.classList.contains('clock-li-readonly')) {
        timeElement.classList.remove('clock-li-readonly');
        let hour = timeElement.value.split(':')[0].slice(0, 2);
        let min = timeElement.value.split(':')[1].slice(1, 3);
        timeElement.value = hour + ':' + min;
        let editCmd = targetElement.querySelector('[name="editCmd"]');
        ...
        if (domain !== '' && time !== '') {
            ...
            let resultTime = convertShortSummaryTimeToString(convertTimeToSummaryTime(time));
