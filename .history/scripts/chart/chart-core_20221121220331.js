function donutChart() {
    let width,
        ...
        letiable, // value in data that will dictate proportions on chart
    ...
    function chart(selection) {
        selection.each(function (data) {
            ...
            let pie = d3.pie()
                .value(function (d) { return floatFormat(d[letiable]); })
                .sort(null);
            ...
            let legendG = svg.selectAll(".legend") // note appending it to mySvg and not svg to make positioning easier
                .data(pie(data))
                .enter().append("g")
    ...
    chart.letiable = function (value) {
        if (!arguments.length) return letiable;
        letiable = value;
        return chart;
    };
...
function barChart(data) {
    let margin = { top: 25, right: 5, bottom: 25, left: 5 },
        width = 555,
        height = 160;
...
function drawIntervalChart(data) {
    data.forEach(function (item) {
        let hFrom = getHourFrom(item.interval);
        let hTo = getHourTo(item.interval);
        if (hFrom != hTo) {
            let sourceTimeFrom = item.interval.split('-')[0].split(':');
            let sourceTimeTo = item.interval.split('-')[1].split(':');
            let timeTo = sourceTimeFrom[0] + ":" + 59 + ":" + 59;
            let timeFrom = sourceTimeTo[0] + ":" + 00 + ":" + 00;
    ...
    let margin = { top: 5, right: 10, bottom: 20, left: 20 },
    ...
    svg.selectAll(".bar")
        ...
        .attr("height", function (d) {
            let offset = getMinutesTo(d.interval) - getMinutesFrom(d.interval);
            if (offset <= 0) {
                let offsetSeconds = getSecondsTo(d.interval) - getSecondsFrom(d.interval);
                if (offsetSeconds <= 3)
                    return 0;
                else
                    return 1;
    ...
    function getHourFrom(interval) {
        let time = interval.split('-')[0];
        return time.split(':')[0];
    }

    function getHourTo(interval) {
        let time = interval.split('-')[1];
        return time.split(':')[0];
    }
