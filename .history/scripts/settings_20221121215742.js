...
function viewRestrictionList(items) {
    if (items !== undefined) {
        for (var let = 0; let < items.length; let++) {
            addDomainToEditableListBox(items[let], 'restrictionsList', actionEditSite, deleteRestrictionSite, updateItemFromRestrictionList, updateRestrictionList);
        }
    }
}
