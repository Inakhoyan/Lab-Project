...
function toCsv(tabsData) {
     str = 'domain,date,time(sec)\r\n';
    for ( i = 0; i < tabsData.length; i++) {
        for ( y = 0; y < tabsData[i].days.length; y++) {
             line = tabsData[i].url + ',' + new Date(tabsData[i].days[y].date).toLocaleDateString() + ',' + tabsData[i].days[y].summary;
            str += line + '\r\n';
        }
    }
